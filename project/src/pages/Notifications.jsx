...
const Notifications = () => {
  ...
  const handleMarkAll = async () => {
    try {
      ...
      setError(e?.message || t('notifications.mark_error'));
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-5xl mx-auto p-6">
        ...
        {loading && <p className="text-gray-600">{t('notifications.loading')}</p>}
        {error && <p className="text-red-600 mb-4">{error}</p>}
          ...
          {items.map((n) => (
            <div
              ...
              className={`bg-white rounded-lg shadow p-4 border ${
                n.is_read ? 'border-gray-200' : 'border-green-300'
              }`}
            >
              <div className="flex items-start justify-between">
                <div>
              ...
              <div className="mt-3 text-sm text-gray-700">
                {n.buyer_name && (
                  <div>
                    <strong>{t('notifications.buyer')}:</strong> {n.buyer_name}
                  </div>
                )}
                {n.buyer_email && (
                  <div>
                    <strong>{t('notifications.email')}:</strong>{' '}
                    <a
                      className="text-blue-600 hover:underline"
                      href={`mailto:${n.buyer_email}`}
                    >
                      {n.buyer_email}
                    </a>
                  </div>
                ...
                {n.buyer_phone && (
                  <div>
                    <strong>{t('notifications.phone')}:</strong>{' '}
                    <a
                      className="text-blue-600 hover:underline"
                      href={`tel:${n.buyer_phone}`}
                    >
                      {n.buyer_phone}
                    </a>
                  </div>
