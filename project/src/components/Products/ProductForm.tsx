...
export function ProductForm({ open, onOpenChange, onProductAdded }: ProductFormProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold text-gray-800">{t('sell_equipment')}</DialogTitle>
          <p className="text-sm text-gray-500">{t('fill_details')}</p>
        </DialogHeader>

        <form onSubmit={handleSubmit} className="space-y-6">
          {/* Image Upload */}
          <div className="space-y-2">
            <Label className="text-sm font-medium text-gray-700">{t('equipment_photos')}</Label>
            <div className="flex flex-wrap gap-4">
              {previews.map((preview, index) => (
                <div key={index} className="relative group">
                  <img src={preview} alt={`Preview ${index + 1}`} className="w-24 h-24 object-cover rounded-md border" />
              ...
              {previews.length < 8 && (
                <label className="flex flex-col items-center justify-center w-24 h-24 border-2 border-dashed border-gray-300 rounded-md cursor-pointer hover:border-green-500 transition-colors bg-gray-50">
                  <Upload className="w-5 h-5 text-gray-400 mb-1" />
                  <span className="text-xs text-gray-500 text-center px-2">{t('add_photo')}</span>
                  <input type="file" className="hidden" accept="image/*" multiple onChange={handleImageChange} />
                </label>
              )}
            </div>
            <p className="text-xs text-gray-500">{t('main_display_note')}</p>
          </div>

          {/* Product Details */}
          <div className="space-y-6">
            <div className="space-y-2">
              <Label htmlFor="title">{t('equipment_title')}</Label>
              <Input id="title" name="title" value={formData.title} onChange={handleInputChange} required />
              <p className="text-xs text-gray-500">{t('equipment_title_hint')}</p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-gray-500">{t('description_hint')}</p>
            </div>
          </div>
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-gray-200">
            ...
            <Button type="submit" className="bg-green-600 hover:bg-green-700" disabled={isSubmitting}>
              {isSubmitting ? t('publishing') : t('publish_listing')}
            </Button>
          </div>
        </form>
      </DialogContent>
